...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        if(result.height == 480)
        {
            ...
            if(version > 6 ) {
                imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 70, frame.size.width, frame.size.height+20)];
            }
            else {
                imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 10, frame.size.width, frame.size.height)];
            }
        ...
        if(result.height == 568)
        {
            NSInteger version=[[[UIDevice currentDevice] systemVersion] integerValue];
            if(version > 6 ) {
                imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 30, frame.size.width, frame.size.height+20)];
            }
            else {
                imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 10, frame.size.width, frame.size.height)];
            }
...
- (void)setFrame:(CGRect)theFrame
{
    ...
    if(ver >6){
        imageView.frame = CGRectMake( imagePoint.x, imagePoint.y-10, theFrame.size.width * self.zoomScale, theFrame.size.height+12 * self.zoomScale);
    }
    else{
