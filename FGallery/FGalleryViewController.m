...
#define kThumbneilSize 75
#define kThumbnailSpacing 5
#define kThumbneilSpacing 4
...
- (void)loadView
{
	...
	//UIImage *leftIcon = [UIImage imageNamed:@"menu"];
	//UIImage *rightIcon = [UIImage imageNamed:@"menu"];
	//_nextButton = [[UIBarButtonItem alloc] initWithImage:nil style:UIBarButtonItemStylePlain target:self action:@selector(next)];
	//_prevButton = [[UIBarButtonItem alloc] initWithImage:nil style:UIBarButtonItemStylePlain target:self action:@selector(previous)];
    
    Menubtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [Menubtn setBackgroundImage:[UIImage imageNamed:@"menu_button_white"] forState:UIControlStateNormal];
    Menubtn.frame = CGRectMake(00.0, 10.0, 44.0, 49.0);
    [self.view addSubview:Menubtn];
    UIButton *seeAll = [[UIButton alloc] initWithFrame:CGRectMake(280, 20, 23, 23)];
    
    
    [seeAll addTarget:self action:@selector(handleSeeAllTouch:) forControlEvents:UIControlEventTouchUpInside];
    [seeAll setImage:[UIImage imageNamed:@"seeall.png"] forState:UIControlStateNormal];
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIButton *seeAll = [[UIButton alloc] initWithFrame:CGRectMake(280, 10, 23, 23)];
            
            [seeAll addTarget:self action:@selector(handleSeeAllTouch:) forControlEvents:UIControlEventTouchUpInside];
            [seeAll setBackgroundImage:[UIImage imageNamed:@"seeall"] forState:UIControlStateNormal];
           // [self.view addSubview:seeAll];
...
- (void)positionScroller
{
	...
	if( self.interfaceOrientation == UIInterfaceOrientationPortrait || self.interfaceOrientation == UIInterfaceOrientationPortraitUpsideDown )
	{//portrait
		scrollerRect = CGRectMake( 0, 10, screenFrame.size.width, screenFrame.size.height );
	}
	else
	{//landscape
		scrollerRect = CGRectMake( 0, 10, screenFrame.size.height, screenFrame.size.width );
	}
...
- (void)resizeThumbView
{
    ...
    NSInteger ver=[[[UIDevice currentDevice] systemVersion]integerValue];
    if(ver > 6){
        _thumbsView.frame = CGRectMake( 0, barHeight+50, _container.frame.size.width, _container.frame.size.height-barHeight );
    }else{
...
- (void)arrangeThumbs
{
	...
	for (i = 0; i < count; i++) {
        ...
        if(version > 6){
            ...
            thumbView.frame = CGRectMake( dx, dy, kThumbneilSize, kThumbneilSize);
            dx += kThumbneilSize + kThumbneilSpacing;
            
            // check if we need to move to a different row
            if( dx + kThumbneilSize + kThumbneilSpacing > _thumbsView.frame.size.width - kThumbneilSpacing )
            {
                dx = 0.0;
                dy += kThumbneilSize + kThumbneilSpacing;
            }
        }
		// increment position
...
- (void)hideThumbnailViewWithAnimation:(BOOL)animation
{
    _isThumbViewShowing = NO; UIButton *seeAll = [[UIButton alloc] init];
    NSInteger ver=[[[UIDevice currentDevice]systemVersion ] integerValue];
    if(ver > 6){
        seeAll = [[UIButton alloc] initWithFrame:CGRectMake(280, 30, 23, 23)];
    }
    else{
        seeAll = [[UIButton alloc] initWithFrame:CGRectMake(280, 10, 23, 23)];
    }
    [seeAll addTarget:self action:@selector(handleSeeAllTouch:) forControlEvents:UIControlEventTouchUpInside];
    [seeAll setBackgroundImage:[UIImage imageNamed:@"seeall.png"] forState:UIControlStateNormal];
    UIBarButtonItem *btn = [[UIBarButtonItem alloc] initWithCustomView:seeAll];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    FGalleryViewController *galleryController = [[FGalleryViewController alloc] init];
